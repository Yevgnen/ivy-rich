...
(defun ivy-rich-bookmark-type (candidate)
  (cl-labels ((propertize-type (string face)
			       (propertize (format "%-8.8s" string) 'face face)))
    (let ((filename (ivy-rich-bookmark-filename candidate)))
      (cond ((null filename)
	     (apply #'propertize-type (or (ivy-rich-bookmark-handler-props candidate)
					  '("NOFILE" 'warning))))
	    ((file-remote-p filename)
	      (propertize-type "REMOTE" 'mode-line-buffer-id))
	     ((not (file-exists-p filename))
	      (apply #'propertize-type (or (ivy-rich-bookmark-handler-props candidate)
					   '("NOTFOUND" 'error))))
	     ((file-directory-p filename)
	       (propertize-type "DIRED" 'warning))
	      (t (propertize-type "FILE" 'success))))))
